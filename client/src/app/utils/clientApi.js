...
export const clientApi = {
  ...
  fetchCollectionsByCategory: (category, opts = {}) => {
    const { page = 1, limit = 12, sort = 'popular', filter = 'all' } = opts;
    return request(`/api/prompt-collections/category/${encodeURIComponent(category)}?page=${page}&limit=${limit}&sort=${sort}&filter=${filter}`);
  },
  fetchCollectionsByType: (type, page = 1) => request(`/api/prompt-collections/type/${type}?page=${page}&limit=12`),
  fetchCollectionsByTag: (tag, page = 1) => request(`/api/prompt-collections/tag/${encodeURIComponent(tag)}?page=${page}&limit=12`),
  fetchSimilarPrompts: (id, tags = [], limit = 4) => request(`/api/prompt-collections/similar/${id}?tags=${encodeURIComponent(tags.join(','))}&limit=${limit}`),
