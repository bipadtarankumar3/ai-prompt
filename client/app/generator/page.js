  ...
  useEffect(() => {
    ...
    async function loadDynamicConfig() {
      try {
        ...
        if (models.length > 0) {
          const defaultModel = models.find(m => m.provider === 'gemini') || 
                               models.find(m => m.provider === 'openai') || 
                               models[0];
          setSelectedModel(defaultModel);
          setProvider(defaultModel.provider);
          if (defaultModel.provider === 'huggingface') {
            setHfModel(defaultModel.api_model_code);
          }
        }
        const settings = await clientApi.fetchSettings();
        setGlobalAIActive(settings.global_ai_active === 'true');
      } catch (err) {
        console.error('Failed to load active models or settings:', err);
      }
    }
    loadDynamicConfig();
  }, []);
              ...
              <AnimatePresence initial={false}>
                {settingsOpen && (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Active Engine</span>
                      <div className="flex flex-col gap-1.5">
                        {activeModels.length === 0 ? (
                          <div className="text-[10px] text-slate-400 p-2 text-center">No active models.</div>
                        ) : (
                          activeModels.map(m => {
      ...
      <div className="flex-grow h-full flex flex-col overflow-hidden relative">
              ...
              <AnimatePresence>
                {modelDropdownOpen && (
                  ...
                  >
                    ...
                    {activeModels.length === 0 ? (
                      <p className="text-[11px] text-slate-400 p-2 text-center">No active models configured.</p>
                    ) : (
                      activeModels.map(m => {
                        const isSelected = selectedModel && selectedModel.id === m.id;
                        const Icon = m.provider === 'openai' ? Cpu : (m.provider === 'gemini' ? Sparkles : Bot);
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium leading-normal mt-0.5">
                                Provider: {m.provider.toUpperCase()} ({m.api_model_code})
                              </p>
                            </div>
                          </button>
                        );
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/98 via-white/80 to-transparent dark:from-[#0d0d14]/98 dark:via-[#0d0d14]/70 dark:to-transparent pt-12 pb-5 px-4 select-none flex-shrink-0 z-20">
          <div className="max-w-2xl mx-auto w-full flex flex-col gap-2.5">
            {!globalAIActive && (
              <div className="text-center py-2.5 px-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold select-none mb-1 animate-pulse">
            ...
            <form
              onSubmit={handleSend}
              className={`group relative flex flex-col rounded-[10px] border border-slate-200/80 dark:border-[#383838]/80 bg-white/95 dark:bg-[#252525]/95 backdrop-blur-2xl shadow-lg shadow-slate-200/60 dark:shadow-black/30 hover:shadow-xl hover:shadow-slate-200/70 dark:hover:shadow-black/40 transition-all duration-300 focus-within:border-amber-400/60 dark:focus-within:border-amber-500/40 focus-within:shadow-xl focus-within:shadow-amber-500/8 dark:focus-within:shadow-amber-500/5 ${!globalAIActive ? 'opacity-60' : ''}`}
            ...
            >
              ...
              <div className="relative flex items-start gap-1 px-4 pt-3.5 pb-1">
                <textarea
                  ref={textareaRef}
                  value={input}
                  disabled={loading || streamingIdx !== null || !globalAIActive}
                  onChange={e => {
                  ...
                  placeholder={!globalAIActive
                    ? 'AI prompt generation features are temporarily disabled by the admin.'
                    : thread.length === 0
              ...
              <div className="flex items-end justify-between px-3 pb-3 pt-1.5 gap-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    type="submit"
                    disabled={!input.trim() || loading || streamingIdx !== null || !globalAIActive}
